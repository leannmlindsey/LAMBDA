"""
Embedding Analysis Script for GenomeOcean-MoE (GO-MoE)
...
The probe / NN / metric code is identical to the other LAMBDA embedding-based
models (adapted from inference/generanno) so GO-MoE's linear-probe and 3-layer-NN
numbers are directly comparable. The ONLY substantive change is model loading:
GenomeOcean-MoE is loaded natively as a Mixtral model with
`trust_remote_code=False`. The vendored remote-code loading path is broken and
returns incorrect weights on modern transformers, so it must not be used; the
native path reproduces the checkpoint bit-for-bit.

Usage:
    python embedding_analysis.py \
        --csv_dir  /path/to/LAMBDA_v1/train_val_test/2k \
        --model_path /path/to/hf_moe_upcycled \
        --output_dir ./results/go_moe/2k
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="hf_moe_upcycled",
        help="Path to the GenomeOcean-MoE checkpoint (loaded natively as Mixtral, "
             "trust_remote_code=False). On Perlmutter: "
             "/global/cfs/cdirs/jgirnd/projects/LLMs/genomeocean-moe/100m-juhno/checkpoints/hf_moe_upcycled",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="Maximum sequence length in tokens. GO-MoE's BPE tokenizer is ~4.89 "
             "bp/token, so 2048 tokens covers up to ~10 kb (all of LAMBDA's 2k/4k/8k "
             "windows fit without truncation).",
    )
    parser.add_argument(
        "--pooling",
        type=str,
...
def main():
    ...
    print("\n" + "=" * 60)
    print("GenomeOcean-MoE (GO-MoE) Embedding Analysis")
    print("=" * 60)
    ...
    if os.path.exists(embeddings_path):
        ...
        model = AutoModel.from_pretrained(args.model_path, trust_remote_code=False)
        model = model.to(device)
